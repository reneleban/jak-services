...
docker-update-card:
	...
	docker rmi jak-card
	docker build -t jak-card -f card.docker .;
	docker run -d -v jak-card-storage:/application -p 10020:10020 --name jak-card jak-card
...
docker-update-login:
	...
	docker build -t jak-login -f login.docker .;
	docker run -d -v jak-login-storage:/application -p 10030:10030 --name jak-login jak-login
...
docker-update-list:
	...
	docker rmi jak-list
	docker build -t jak-list -f list.docker .;
	docker run -d -v jak-list-storage:/application -p 10010:10010 --name jak-list jak-list
