...
docker-update-card:
	...
	docker rmi jak-card
	docker build -t jak-card -f card.docker .;
	docker run -d -v jak-card-storage:/application/data -p 10020:10020 --name jak-card jak-card
...
docker-update-login:
	...
	docker build -t jak-login -f login.docker .;
	docker run -d -v jak-login-storage:/application/data -p 10030:10030 --name jak-login jak-login
...
docker-update-list:
	...
	docker rmi jak-list
	docker build -t jak-list -f list.docker .;
	docker run -d -v jak-list-storage:/application/data -p 10010:10010 --name jak-list jak-list
...
docker-update-board:
	...
	docker build -t jak-board -f board.docker .;
	docker run -d -v jak-board-storage:/application/data -p 10000:10000 --name jak-board jak-board
